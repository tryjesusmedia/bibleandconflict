...
The app reads and writes only the signed-in user's rows. Public leaderboard output comes only from the curated `get_conflict_journey_leaderboard` RPC, which exposes rank, random alias, points, completed-item count, and whether the row belongs to the caller. Email, user ID, avatar, and private welcome name are not part of the leaderboard model.
