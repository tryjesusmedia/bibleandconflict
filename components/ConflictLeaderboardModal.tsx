...
import type { ConflictLeaderboardEntry } from '@/lib/conflictRewardsCore';

type ConflictLeaderboardModalProps = {
  visible: boolean;
  signedIn: boolean;
  aliasSaving: boolean;
  signInBusy: boolean;
...
export function ConflictLeaderboardModal({
  visible,
  signedIn,
  aliasSaving,
  signInBusy,
  ...
  useEffect(() => {
    ...
    void getConflictLeaderboard().then((rows) => {
      if (generation !== loadGenerationRef.current) return;
      setEntries(rows);
      setHasLoaded(true);
      setLoadError(false);
    ...
    <View style={styles.headerContent}>
      <View style={styles.titleRow}>
        <View style={styles.titleCopy}>
          <Eyebrow>BIBLE & CONFLICT COMMUNITY</Eyebrow>
...
const styles = StyleSheet.create({
  ...
  changeNameText: { color: colors.gold, fontSize: 13, lineHeight: 18, fontWeight: '900' },
