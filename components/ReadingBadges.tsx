...
import { conflictPlan, conflictReadingById, conflictReadingComplete, type ConflictReading } from '@/data/conflictPlan';
...
export function ReadingBadgeProvider({ children }: { children: React.ReactNode }) {
  const { session } = useAuth();
  const pathname = usePathname();
  const identity = `${session?.user.id ?? 'guest'}:${pathname}`;
  const { completed, ready } = useConflictJourney();
  ...
  if (selected) {
    const reading = conflictReadingById.get(selected.id);
    if (openedIdentity !== identity || !ready || !reading || !conflictReadingComplete(reading, completed)) setSelected(null);
  }
  useEffect(() => {
    if (!selected) return;
  ...
  const open = useCallback((badge: ReadingBadge) => {
    const reading = conflictReadingById.get(badge.id);
    if (ready && reading && conflictReadingComplete(reading, completed)) {
