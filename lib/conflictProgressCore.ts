...
export function hasConflictAccountData(
  ...
  const primaryHasJourneyActivity = Boolean(
    primaryProgress
    && (primaryProgress.completed.length > 0 || primaryProgress.lastIndex > 0),
  );
  return primaryHasJourneyActivity || settingIndex > 0 || legacyActivityExists;
}
...
export function shouldLinkGuestConflictProgress(
  ...
  const firstLink = !pendingLinkUserId && !accountLocal && !remote;
